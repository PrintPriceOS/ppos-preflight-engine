...
const OUTPUT_DIR = path.resolve(__dirname, '../fixtures/phase61e_out');
const REPORTS_DIR = path.resolve(__dirname, '../reports');

async function run() {
    await fs.ensureDir(OUTPUT_DIR);
    
    const engine = new AutofixExecutionEngine({ jobId: 'smoke_61e' });
    const results = [];
    ...
    const mdReport = [
        ...
        '',
        '## Executive Summary',
        hasFailures ? 'Regression test FAILED.' : 'Regression test PASSED.',
        '',
        '## Capability Matrix',
    ...
    await fs.writeJson(path.join(REPORTS_DIR, 'phase61e_engine_structural_metadata_regression.json'), results, { spaces: 2 });
    await fs.writeFile(path.join(REPORTS_DIR, 'phase61e_engine_structural_metadata_regression.md'), mdReport);

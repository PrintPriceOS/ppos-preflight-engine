'use strict';
/**
 * Phase 63E.1 Smoke Test — Engine PDF Security/Interactivity End-to-End Regression
 *
 * Reuses the Phase 63A fixtures (fixtures/phase63a/*) and harness
 * (AutofixExecutionEngine, checkGovernance/checkOutputValidity helpers) to
 * re-run the full security/interactivity capability matrix and confirm that
 * governance, evidence, and output-validity guarantees still hold end-to-end
 * before the result is handed downstream to Worker 63E.2.
 */
...
async function runFix(engine, capability, fixture) {
    ...
    const outputPath = path.join(FIXTURES_DIR, `phase63e_regression_output_${capability.toLowerCase()}_${fixture}`);
    try {
...
async function main() {
    ...
    const scenarios = [
        { name: 'STRIP_JAVASCRIPT removes or skips honestly', fixture: 'javascript_action.pdf', capability: 'STRIP_JAVASCRIPT', allowedStatuses: ['APPLIED', 'SKIPPED', 'SKIPPED_UNSUPPORTED'] },
        { name: 'REMOVE_LAUNCH_ACTIONS removes or skips honestly', fixture: 'launch_action.pdf', capability: 'REMOVE_LAUNCH_ACTIONS', allowedStatuses: ['APPLIED', 'SKIPPED', 'SKIPPED_UNSUPPORTED'] },
        { name: 'REMOVE_EMBEDDED_FILES removes or skips honestly', fixture: 'embedded_file.pdf', capability: 'REMOVE_EMBEDDED_FILES', allowedStatuses: ['APPLIED', 'SKIPPED', 'SKIPPED_UNSUPPORTED'] },
        { name: 'REMOVE_DOCUMENT_OPEN_ACTIONS removes or skips honestly', fixture: 'document_open_action.pdf', capability: 'REMOVE_DOCUMENT_OPEN_ACTIONS', allowedStatuses: ['APPLIED', 'SKIPPED', 'SKIPPED_UNSUPPORTED'] },
        { name: 'REMOVE_PAGE_OPEN_ACTIONS removes or skips honestly', fixture: 'page_open_action.pdf', capability: 'REMOVE_PAGE_OPEN_ACTIONS', allowedStatuses: ['APPLIED', 'SKIPPED', 'SKIPPED_UNSUPPORTED'] },
        { name: 'FLATTEN_ANNOTATIONS applies only if safe, otherwise SKIPPED_UNSUPPORTED', fixture: 'annotations_basic.pdf', capability: 'FLATTEN_ANNOTATIONS', allowedStatuses: ['APPLIED', 'SKIPPED_UNSUPPORTED'] },
        { name: 'FLATTEN_FORMS applies only if safe, otherwise SKIPPED_UNSUPPORTED', fixture: 'acroform_basic.pdf', capability: 'FLATTEN_FORMS', allowedStatuses: ['APPLIED', 'SKIPPED_UNSUPPORTED', 'NO_CHANGE'] },
        { name: 'mixed_interactive_content preserves evidence (STRIP_JAVASCRIPT)', fixture: 'mixed_interactive_content.pdf', capability: 'STRIP_JAVASCRIPT', allowedStatuses: ['APPLIED', 'SKIPPED', 'SKIPPED_UNSUPPORTED'], requireEvidence: true },
        { name: 'mixed_interactive_content preserves evidence (FLATTEN_FORMS)', fixture: 'mixed_interactive_content.pdf', capability: 'FLATTEN_FORMS', allowedStatuses: ['APPLIED', 'SKIPPED_UNSUPPORTED', 'NO_CHANGE'], requireEvidence: true },
        { name: 'clean_control returns no action with evidence (STRIP_JAVASCRIPT)', fixture: 'clean_control.pdf', capability: 'STRIP_JAVASCRIPT', allowedStatuses: ['APPLIED', 'NO_CHANGE', 'SKIPPED'], requireEvidence: true },
        { name: 'clean_control returns no action with evidence (FLATTEN_FORMS)', fixture: 'clean_control.pdf', capability: 'FLATTEN_FORMS', allowedStatuses: ['NO_CHANGE', 'SKIPPED', 'SKIPPED_UNSUPPORTED'], requireEvidence: false }
    ];
    ...
    // Standards overclaim regression — scan all results for any forbidden claim
    {
        ...
        results.push({
            ...
            pass: passed,
            notes: notes.length ? notes : ['No security/interactivity fix produced a standards, PDF/X, PDF/A, or production certification claim end-to-end.']
        });
    }
    ...
    const reportJson = {
        ...
        core_principle: 'End-to-end: security/interactivity cleanup must never imply PDF/X compliance, PDF/A compliance, standards certification, production certification, print-ready status, or certified.pdf trust — from Engine through Control Plane.',
    ...
    await fs.writeJson(path.join(REPORTS_DIR, 'phase63e_engine_security_interactivity_regression.json'), reportJson, { spaces: 2 });
    ...
    md.push('', '## Forbidden Overclaims Checked', ...reportJson.forbidden_overclaims_checked.map(c => `- ${c}`));

    await fs.writeFile(path.join(REPORTS_DIR, 'phase63e_engine_security_interactivity_regression.md'), md.join('\n'));

    console.log(`\nPhase 63E.1 Engine regression — smoke_passed: ${smokePassed}`);
